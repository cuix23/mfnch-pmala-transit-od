...
def pmala_update_phi(
    ...
    """
    Block-wise pMALA for Phi with diagonal preconditioner.
    ...
    u : (N, S)
    sigma0 : float
    row_slices : list of slices
    h_phi : float or array of shape (n_blocks,)
        Step size for each block (if array) or common step size.
    rng : numpy.random.Generator
    M_mode : str, {"none", "k_scaled", "simple", "psi_scaled"}
    M_inflate : float
    m_min, m_max : float or None
        Clip the diagonal preconditioner.
    h_min, h_max : float
        Clipping range for step sizes (used in adaptation, not inside this function).

    Returns
    -------
    Phi_new : (M, D)
    block_accept_cum : (n_blocks,) int
        Cumulative acceptance counts (since start).
    block_trials_cum : (n_blocks,) int
        Cumulative trial counts (since start).
    block_accepted_now : (n_blocks,) int
        Acceptance indicator (0/1) for this iteration.
    current_logpost : float
        Log-posterior after update.
    """
    M, D = Phi.shape
    N = Psi.shape[0]
    n_blocks = len(row_slices)

    # ----- global lambda and current log-posterior -----
    G = compute_G(Phi, Psi)
    lam = compute_lambda(G, row_slices)
    current_logpost = logposterior_phi(Phi, Psi, Y, u, row_slices, sigma0)
    ...
    # ----- per‑iteration acceptance flags -----
    ...
    for idx in order:
        ...
        Phi_block_old = Phi_new[sl, :].copy()      # (K_i, D)
        lam_block_old = lam[sl, :].copy()          # (K_i, N)

        Y_block = Y[:, sl]                         # (N, K_i)
        u_i = u[:, i]                             # (N,)

        # ---- gradient at old state ----
        R_i_old = Y_block - u_i[:, None] * lam_block_old.T
        grad_i_old = R_i_old.T @ Psi - Phi_block_old / (sigma0 ** 2)

        # ---- diagonal preconditioner at old state ----
        if M_mode == "none":
            M_block = np.ones_like(Phi_block_old)
        else:
            M_block, _ = precond_M_block(
                lam_block=lam_block_old,
                u_i=u_i,
                sigma0=sigma0,
                Psi=Psi,
                mode=M_mode,
        ...
        # ---- forward proposal ----
        mean_fwd = Phi_block_old + 0.5 * h_i * (M_block * grad_i_old)
        sd_fwd = np.sqrt(h_i) * np.sqrt(np.maximum(M_block, eps))
        noise = rng.normal(size=grad_i_old.shape)
        Phi_block_prop = mean_fwd + sd_fwd * noise

        # ---- proposed lambda ----
        Phi_new[sl, :] = Phi_block_prop
        G_block_prop = Phi_block_prop @ Psi.T
        G_block_shift = G_block_prop - G_block_prop.max(axis=0, keepdims=True)
        E = np.exp(G_block_shift)
        lam_block_prop = E / (E.sum(axis=0, keepdims=True) + eps)

        # ---- log-likelihood contributions ----
        Y_block_T = Y_block.T
        loglik_old_block = np.sum(Y_block_T * np.log(lam_block_old + eps))
        loglik_new_block = np.sum(Y_block_T * np.log(lam_block_prop + eps))

        # ---- prior contributions ----
        prior_old_block = -0.5 / (sigma0 ** 2) * np.sum(Phi_block_old ** 2)
        prior_new_block = -0.5 / (sigma0 ** 2) * np.sum(Phi_block_prop ** 2)

        # ---- gradient at proposed state (for backward q) ----
        R_i_new = Y_block - u_i[:, None] * lam_block_prop.T
        grad_i_new = R_i_new.T @ Psi - Phi_block_prop / (sigma0 ** 2)

        # ---- preconditioner at proposed state ----
        if M_mode == "none":
            M_block_p = np.ones_like(Phi_block_prop)
        else:
            M_block_p, _ = precond_M_block(
                lam_block=lam_block_prop,
                u_i=u_i,
                sigma0=sigma0,
                Psi=Psi,
                mode=M_mode,
        ...
        def _log_q_block(from_block, grad_from, to_block, M_blk, h):
        ...
        logq_fwd = _log_q_block(Phi_block_old, grad_i_old, Phi_block_prop, M_block, h_i)
        logq_bwd = _log_q_block(Phi_block_prop, grad_i_new, Phi_block_old, M_block_p, h_i)
        ...
        if np.log(rng.uniform()) < logr:
            ...
            # reject
            Phi_new[sl, :] = Phi_block_old
            # lam[sl, :] remains old (already correct)
